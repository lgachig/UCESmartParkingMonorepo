...
import { enrichSlotsWithOccupancy, type EnrichedSlot } from '@/lib/report-metrics';
...
export default function AdminReportsPage() {
  ...
  const [enrichedData, setEnrichedData] = useState<EnrichedSlot[]>([]);
  const [enrichingLoading, setEnrichingLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    enrichSlotsWithOccupancy(slots)
      .then((result) => {
        if (!cancelled) setEnrichedData(result);
      })
      .catch((err) => {
        console.error('Error enriching slots:', err);
        if (!cancelled) {
          setEnrichedData(slots.map((s) => ({ ...s, alertasActivas: [], tiempoH: '0.0' })));
        }
      })
      .finally(() => {
        if (!cancelled) setEnrichingLoading(false);
      });
    return () => {
      cancelled = true;
    };
        ...
        <div className="flex-none sticky top-0 z-10 bg-[#f8fafc] pb-2 -mx-3 md:-mx-6 lg:-mx-8 px-3 md:px-6 lg:px-8 pt-0">
          <div className="bg-white p-5 lg:p-8 rounded-[2rem] lg:rounded-[3.5rem] shadow-sm border-l-[10px] lg:border-l-[15px] border-[#003366] flex flex-col lg:flex-row justify-between items-center gap-4 lg:gap-6">
            <div className="text-center lg:text-left w-full lg:w-auto">
              ...
              <p className="text-[10px] lg:text-sm font-bold text-gray-400 mt-1 uppercase tracking-[0.3em] flex items-center justify-center lg:justify-start gap-2">
                Tiempo Real {enrichingLoading && <Loader2 size={12} className="animate-spin" />}
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-4 mt-2">
                      <div
                        className={`w-12 h-12 lg:w-16 lg:h-16 rounded-xl flex items-center justify-center text-2xl lg:text-3xl font-black shadow-inner ${isOccupied || isReserved ? 'bg-[#003366] text-white' : 'bg-white text-gray-300'
                          }`}
                      >
                        {slot.number}
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <span
                          className={`px-2 py-1 rounded-md text-[9px] lg:text-[10px] font-black uppercase ${isOccupied
