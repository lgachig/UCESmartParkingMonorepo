...
export default function SystemHealthPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`p-6 xl:p-8 rounded-3xl border transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white ${
        isOperational ? 'border-emerald-100 shadow-emerald-50/10' : 'border-amber-100 shadow-amber-50/10'
      }`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] xl:text-xs font-black uppercase tracking-wider ${
        ...
        <div className="flex items-center gap-3 bg-gray-50 hover:bg-gray-100/70 transition-colors px-5 py-3 rounded-2xl border border-gray-100">
          <div className="relative flex h-3 w-3">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
              status.realtime.connected ? 'bg-emerald-400' : 'bg-red-400'
            }`}></span>
            <span className={`relative inline-flex rounded-full h-3 w-3 ${
              status.realtime.connected ? 'bg-emerald-500' : 'bg-red-500'
            }`}></span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="bg-white p-6 xl:p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {status.services.map((service) => (
            <div
              key={service.name}
              className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col justify-between h-28 group ${
                service.up
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                  service.up ? 'bg-emerald-500 shadow-sm animate-pulse' : 'bg-red-500 animate-ping'
                }`} />
              </div>
              <div className="mt-2">
                <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                  service.up ? 'bg-emerald-50 text-emerald-700' : 'bg-red-100 text-red-700'
                }`}>
