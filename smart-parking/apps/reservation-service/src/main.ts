...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Smart Parking — Reservation Service')
    .setDescription('Manages the full reservation lifecycle')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  const port = configService.get<number>('RESERVATION_SERVICE_PORT') || 3005;
  await app.listen(port);
  Logger.log(`🚀 Reservation Service running on: http://localhost:${port}/api`);
  Logger.log(`📚 Swagger docs: http://localhost:${port}/docs`);
  Logger.log(`📊 Metrics: http://localhost:${port}/metrics`);
  Logger.log(`📝 Logs: logs/reservation-service.log`);
}
